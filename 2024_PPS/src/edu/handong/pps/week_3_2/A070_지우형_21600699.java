...
public class A070_지우형_21600699 {
    public static void main(String[] args) {
        ...
        // 큐에 1부터 N까지의 카드 번호를 추가
        for (int i = 1; i <= N; i++) {
            queue.offer(i);//queue의 뒷쪽에 원소 추
        }

        // 카드를 제거하고 밑으로 옮기는 과정
        while (queue.size() > 1) {
            queue.poll();  // 제일 위에 있는 카드를 버림
            int topCard = queue.poll();// 두번째에 있던 카
            queue.offer(topCard);  // 제일 위에 있는 카드를 밑으로 옮김
        }
