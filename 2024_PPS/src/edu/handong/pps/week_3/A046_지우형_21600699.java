...
그는 선수의 이름을 기억하지 못하고, 각 선수의 능력도 알지 못한다. 따라서, 
...
public class A046_지우형_21600699 {
    public static void main(String[] args) {
    	...
    	for(int i=0;i<N;i++) {
    		...
    		for(int j=0;j<26;j++) {
    			if(player[i].charAt(0)==alphabet[j]) {
    				alphabet_num[j]++;
    			}
    		}
    	}
    	
    	boolean isMoreOrLessThanFive = false;
    	for(int i=0;i<26;i++) {
    		if(alphabet_num[i]>=5) {
    			isMoreOrLessThanFive = true;
    			break;
    		}
    	}
    	if(isMoreOrLessThanFive) {
    		for(int i=0;i<26;i++) {
        		if(alphabet_num[i]>=5) {
